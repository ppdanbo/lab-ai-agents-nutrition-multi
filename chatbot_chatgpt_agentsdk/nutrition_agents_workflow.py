...
topic_classifier = Agent(
  ...
  instructions="Classify the subject of the input as either \"calories\" or \"other\", and output a JSON object with the key \"topic\" set to \"calories\" if the input is about calories, or \"other\" if it is not",
  model="gpt-5-nano",
...
no_offtopic_agent = Agent(
  ...
  instructions="Politely inform the user that this workflow is focused exclusively on nutrition topics and cannot assist with non-nutrition-related inquires.",
  model="gpt-5-nano",
  model_settings=ModelSettings(
    store=True,
    reasoning=Reasoning(
...
async def run_workflow(workflow_input: WorkflowInput):
  with trace("2-simple-nutrition-agent"):
    state = {
    ...
    if guardrails_hastripwire:
      no_offtopic_agent_result_temp = await Runner.run(
        ...
        run_config=RunConfig(trace_metadata={
          ...
          "workflow_id": "wf_694fed19bcf481908462e2f78101fffb0ae7d444e3b87dc6"
        })
      )
